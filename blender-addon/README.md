...
> **📦 Companion Repository**: This works with the [Talon Voice Commands for Blender](https://github.com/tanglisha/talon-voice-to-blender) which provides the voice command definitions.
...
The addon includes a status panel that displays Blender state information and the last executed command. Access it by:

1. Open the 3D View sidebar (press `N`)
2. Look for the **"Talon"** tab
3. The panel shows configurable information

### Configurable Display Options

Configure what information to display in **Edit → Preferences → Add-ons → Talon Voice Control**:

- ✓ **Current Mode** (default: enabled) - Shows the current interaction mode (Object, Edit, Sculpt, etc.)
- **Active Object** (default: disabled) - Shows the name of the active object
- **Selection Count** (default: disabled) - Shows number of selected objects
- **Scene Statistics** (default: disabled) - Shows vertex/face/triangle counts
- ✓ **Last Command** (default: enabled) - Shows the last voice command executed with timestamp

### Default Display

By default, the status panel shows minimal information:
- Current mode
- Last command executed (with time elapsed)

This provides quick feedback without cluttering the interface.
